...
- **삽입 위치**:
  ...
  2.  **최종 응답 전 (Pre-Final Response)**: 사용자에게 응답을 전달하기 직전에, 내용의 정확성, 문체의 일관성, 유해성 여부 등을 종합적으로 평가합니다. 이는 에이전트의 신뢰도와 직결되는 중요한 단계입니다.
  3.  **계획 수정 시 (During Reflection)**: 에이전트가 실패를 경험했을 때, Evaluator가 "왜 실패했는가?"를 분석하고 평가하여, Reflection 과정에서 더 나은 계획을 세우는 데 활용될 수 있습니다.
- **구현 방법**:
...
- **결합 방법 (Memory-Augmented RAG)**:
  ...
  2.  **검색 (Retrieval)**: 이렇게 확장된 검색어를 사용하여 벡터 DB에서 관련 문서를 검색합니다.
  3.  **생성 (Generation)**: 검색된 문서와 원본 질문을 함께 LLM에 전달하여, 맥락을 이해하고 사실에 기반한 정확한 답변을 생성합니다.
...
- **설계 방안**:
  ...
  3.  **구현**: 이러한 로직은 주로 **Orchestration Layer**에서 관리됩니다. 오케스트레이터가 라우터의 출력을 받은 뒤, 신뢰도를 체크하고 조건에 따라 다음 단계를 분기(Branching)시킵니다.
...
- [컨텍스트 관리 및 압축 (Context Compression)](../5-2-메모리-and-컨텍스트-관리/context-compression-management.md)
- [기본 RAG 파이프라인 (Basic RAG)](../5-4-retrieval-augmented-generation-rag/basic-rag-pipeline.md)
- [도구 스키마 설계 (Tool Schemas)](../5-9-보안-and-프로토콜/tool-schemas-jsonrpc-openapi.md)