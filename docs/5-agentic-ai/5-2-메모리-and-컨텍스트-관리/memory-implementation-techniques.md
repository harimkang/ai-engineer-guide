...
| [**LEANN**](https://github.com/yichuan-w/LEANN?utm_source=chatgpt.com) | 온디바이스용 경량 인덱스 | 노트북·개인PC에서 대량 문서 RAG, 프라이버시 우선 시나리오             | 아주 최근 프로젝트라 운영·생태계가 성숙하지 않았을 수 있음. 대규모 분산보단 단일/소수 노드에 적합.          |

> **참고**: 온디바이스/프라이버시 중심 개인 RAG를 목표로 한다면, 최근 공개된 경량 인덱스인 **LEANN**(https://github.com/yichuan-w/LEANN)도 고려할 수 있습니다. 기존 HNSW 기반 인덱스보다 인덱스 크기를 크게 줄이면서도 실사용에 필요한 수준의 recall을 유지하는 것을 목표로 하며, 로컬에서 수천만 단위 문서를 다루는 시나리오에 적합하다. 다만 공개 시점이 최근이라 운영·모니터링 측면은 성숙한 상용/오픈소스 벡터DB에 비해 제한적일 수 있습니다.
